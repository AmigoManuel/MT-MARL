...
/* TODO: Acotación - las siguientes dos funciones resultan de cierta manera ineficientes,
de ser posible sería mejor reemplazar el procedimiento por hashing o utilizar una referencia 
desde la celda al agente que se encuentre en ella, de esta forma se evita la busqueda lineal */

/* Determina si auxcell esta ocupada por un agente y retorna el agente referenciado
...
/* Determina si auxcell es el goal de otro agente y retornando la referencia al agente 
 * @param auxcell celda a determinar referencia
 * @return retorna si la celda se encuentra ocupada o no */
...
/* Despliega el mapa con los agentes, obstaculos, celdas objetivo y celdas vacías por pantalla */
...
int compare_path(int a, int j, int myConflictStep, int otherConflictStep, int lookahead) {
    ...
    if ((myConflictStep < realDepth[a]) && (otherConflictStep < realDepth[j])) {
        printf("Case 1");
        neigh_x = idealPath[j][otherConflictStep + 1]->x - idealPath[j][otherConflictStep]->x;
        neigh_y = idealPath[j][otherConflictStep + 1]->y - idealPath[j][otherConflictStep]->y;
        mine_x = idealPath[a][myConflictStep + 1]->x - idealPath[a][myConflictStep]->x;
        mine_y = idealPath[a][myConflictStep + 1]->y - idealPath[a][myConflictStep]->y;
    }
    ...
    if ((myConflictStep == realDepth[a]) && (otherConflictStep == realDepth[i])) {
        printf("Case 4");
        neigh_x = idealPath[j][otherConflictStep]->x - idealPath[j][otherConflictStep - 1]->x;
        neigh_y = idealPath[j][otherConflictStep]->y - idealPath[j][otherConflictStep - 1]->y;
        mine_x = idealPath[a][myConflictStep]->x - idealPath[a][myConflictStep - 1]->x;
        mine_y = idealPath[a][myConflictStep]->y - idealPath[a][myConflictStep - 1]->y;
    }
...
void determine_constraints(int a, int lookahead, int formula, cell1 *currentCell, int initialState) {
    ...
    int cell_role;
    // Verifica otros agentes por posibles conflictos en la celda actual y tambien por celdas futuras.
    // Por cada uno de los agentes
    for (int j = 0; j < NAGENTS; ++j) {
        ...
        if (a != j) {
            ...
            distanceFromAgent = abs(position[a]->x - position[j]->x) + abs(position[a]->y - position[j]->y);
        }
        ...
        if ((!goal_reached[j]) &&
            (((abs(position[j]->x - position[a]->x) + abs(position[j]->y - position[a]->y)) <= (lookahead)) &&
            ((abs(position[j]->x - position[a]->x) + abs(position[j]->y - position[a]->y)) > 0)) &&
            (a != j)) {
            ...
            if (((formula < 20000) || (agentInfo[j] < 20000)) &&
                ...
                // Analiza el camino del vecino, compara con el camino ideal del vecino j (shared after the previous iteration)
                for (int l = 1; l <= lookahead; l++) {
                    ...
                    if ((a != j) && (idealPath[j][l] != NULL)) {
                        ...
                        printf("Agent %i: [%d %d] me: [%d %d], H: %.1f, step %i \n", j + 1, idealPath[j][l]->y,
                               idealPath[j][l]->x, idealPath[a][l]->y, idealPath[a][l]->x,
                               hvalues[MAZEWIDTH * position[j]->y + position[j]->x][j], l);

                        // Si mi posición actual esta en el camino ideal de mi vecino dentro del paso l (how bad is to stay here)
                        if ((position[a]->y == idealPath[j][l]->y) &&
                            (position[a]->x == idealPath[j][l]->x)) {
                            // Incrementa el numero de conflictos para esta posición por este agente.
                            position[a]->numConflicts[a] = position[a]->numConflicts[a] + 1;
                        ...
                        // Si la siguiente posición en mi camino ideal (desde step=1) y que ambos no sean nulos para ambos de nosotros
                        if ((idealPath[a][l + 1] != NULL) &&
                            (idealPath[j][l + 1] != NULL) &&
                            (l < lookahead)) {

                            // Si existe otro conflicto en el paso siguiente de el plan para ambos (yo y mi vecino j)
                            if ((idealPath[j][l + 1] != NULL) &&
                                (idealPath[a][l + 1]->y == idealPath[j][l + 1]->y) &&
                                (idealPath[a][l + 1]->x == idealPath[j][l + 1]->x)) {

                                // Mi agente a para la siguiente posición se encuentra en el camino el camino del vecino j
                                ...
                                // Basado en esta realación se necesita tomar una decisión
                                conflictStepMe = l + 1;
                                conflictStepOther = l + 1;

                                // Si sobre mi vecino j se encuentra el subsiguiente (l+1 no es NULL)
                            ...
                            // Si mi siguiente posicion se encuentra sobre la posicion del otro agente
                            // podria necesitar a esperar si yo quisiera moverme primero
                            // Otro caso el necesitaria intentar moverse primero y no donde me encuentro ahora
                            if ((idealPath[j][l] != NULL) &&
                                ...
                                // La siguiente posicion de a se encuentra sobre el camino de j
                                printf("Oops! might need to WAIT, OTHER AGENT IN MY WAY at position [%d %d] \n",
                                       idealPath[a][l + 1]->y, idealPath[a][l + 1]->x);
                                printf("My info %i vs other agent's info %i \n", formula, agentInfo[j]);
                                // Basado en esta relación, es necesario tomar una desicion
                                conflictStepMe = l + 1;
                                conflictStepOther = l;

                                // Verifica si la interseccion continua repitiendose a futuro
                                // esto significa que ambos se encuentran siguiendo el mismo camino
                                if (idealPath[a][l + 2] != NULL) {
                                    if ((idealPath[a][l + 1]->y == idealPath[j][l]->y) &&
                                        (idealPath[a][l + 1]->x == idealPath[j][l]->x) &&
                                        (idealPath[j][l + 1]->y == idealPath[a][l + 2]->y) &&
                                        (idealPath[j][l + 1]->y == idealPath[a][l + 2]->y)) {
                                        // TODO: Agent a is following agent j !!! here we could make a team or something TODO
                                    }
                                }
                            }

                            // Si yo y mi vecino j cruzamos caminos (no es factible el cambio)
                            // en el mismo camino pero con direcciones opuestas
                            if ((idealPath[j][l + 1] != NULL) &&
                                (idealPath[a][l + 1]->y == idealPath[j][l]->y) &&
                                (idealPath[a][l + 1]->x == idealPath[j][l]->x) &&
                                ...
                                // Si la siguiente posicion de a forma parte del camino de j
                                printf("Oops! UNFEASIBLE SWAPPING projected at [%d %d] \n", idealPath[a][l + 1]->y,
                                       idealPath[a][l + 1]->x);
                                printf("My info %i vs other agent's info %i \n", formula, agentInfo[j]);
                                // Basado en esta realacion, necesitamos tomar una desicion
                                // Conocemos de inmediato que ellos se encuetran en celdas opuestas
                                conflictType[a][j] = 1;
                                conflictStepMe = l;
                                conflictStepOther = l + 1;
                            }
                ...
                // compare_path(a,j, conflictStepMe, conflictStepOther, lookahead);
                // }
    ...
    // El paso de conflicto por cada agentes (conflictStepMe, conflictStepOther) NO SERA ALMACENADO!! ¿yo quiero esto?
    ...
    // En la seccion anterior, iteramos por cada uno de los pasos en el plan de movimiento,
    // pero esta comienza desde pathlength[a], mientra la otra desde 0
    for (int future = pathlength[a]; future <= lookahead; ++future) {
            ...
            if (((maze1[currentCell->y][currentCell->x].blockedAgent[a][future]) && (future > 0)) ||
                ...
                // El primer caso, para el primer paso (DESDE PATHLENGTH) en el plan, donde otro agente planee moverse a mi posicion
                // Este verifica la posicion actual, cuando los cambios actuales dependen del paso sobre el camino
                if (future == pathlength[a]) {
                    ...
                    // Por cada agente
                    for (int j = 0; j < NAGENTS; ++j) {
                        // TODO: verificar - Si en la celda actual, el agente a se mueve con diferencia a j (existe delta)
                        if ((maze1[currentCell->y][currentCell->x].agentMovingTo[a][future][j] > 0) &&
                            (j != a)) {
                                // Si el otro agente - sobre el paso actual = 0 (significa que se encuentra sobre su objetivo)
                                if ((goal_reached[j])) {
                                ...
                                printf("His previous position at [%d %d] had a degree of %i \n", path[j][future]->y,
                                        path[j][future]->x, maze1[path[j][future]->y][path[j][future]->x].degree[j]);
                                printf("My info %i vs other agent's info %i \n", formula, agentInfo[j]);
                                ...
                                if (conflictType[a][j] == 0) {
                                    ...
                                    //Former comparison, based only on who has higher heuristic
                                ...
                                // Si es un punto de conflicto 1, uno de ambos debe hacerse a un lado
                        ...
                        if ((formula > 20000) &&
                            (agentInfo[j] > 20000)) {
                                ...
                                // Si j quiere moverse a mi posicion actual (En mi actual valor future/pathleght[i])
                                printf("(1) We are both SCREWED!!, his H is %i, mine is %i and my distance from start is %i, dist from agent is %i, total for comparison: Other %i vs mine %i \n",
                            ...
                            for (int l = 1; l <= lookahead; l++) {
                                // Si a es distinto de j y mi camino ideal se ecuentra definido
                                if ((a != j) && (idealPath[j][l] != NULL)) {

                                    printf("Agent %i: [%d %d], H: %.1f, step %i \n", j + 1, idealPath[j][l]->y,
                                            idealPath[j][l]->x, hvalues[MAZEWIDTH * position[j]->y + position[j]->x][j], l);

                                    // Si encuentra una intersección entre a y j sobre la iteración l
                                    if ((position[a]->y == idealPath[j][l]->y) &&
                                        (position[a]->x == idealPath[j][l]->x)) {
                                        // Asigna el contador de conflictos a la posición dada
                            ...
                            if ((hvalues[MAZEWIDTH * (position[j]->y) + (position[j]->x)][j] > maxH)) {
                                // Marca a j como el nuevo agente con mayor valor heuristico
                                maxHagent = j;
...
//HEEY IF IT IS THE SECOND TIME IT STAYS IN THE STATE; IT COUNTS MORE DEGREES!!!!
//Need to save the state so that it can be used for swapping
    ...
    printf(" COMPUTING FOR %i (%i), at [%d %d] w degree %i, lastMobile at %i!!!\n", a + 1, pathlength[a],
           position[a]->y, position[a]->x, maze1[position[a]->y][position[a]->x].degree[a], lastMobileCellDist[a]);
...
//For each cell in the map
        ...
        for (int x = 0; x < MAZEWIDTH; ++x) {
            ...
            for (int z = 0; z < lookahead + 1; ++z) {
                ...
                if (maze1[y][x].blockedAgent[a][z] > 0) {
                    ...
                    if (maze1[y][x].toTransition[a][z] > 0)//(toTransition[x][y][a][z]>0)
                    {
                        printf(" TO [%d %d] at T %i: %i by agents: ", y, x, z, maze1[y][x].toTransition[a][z]);

                        for (int j = 0; j < NAGENTS; ++j) {
                            if (maze1[y][x].agentMovingTo[a][z][j])//(agentMovingTo[x][y][a][z][j]>0)
                            {
                                printf(" %i, ", j + 1);
                            }
                        }
                ...
                //Double loop through the maze, looking for transitions between cells
                for (int q = 0; q < MAZEHEIGHT; ++q) {

                    for (int e = 0; e < MAZEWIDTH; ++e) {

                        if (z > 0) {
                            if (((maze1[y][x].fromTransition[a][z - 1] > 0) && (maze1[q][e].toTransition[a][z])) &&
                                (abs(q - y) + abs(e - x) <= 1))//(betweenTransition[x][y][e][q][a][z]>0)
                            {
                                for (int j = 0; j < NAGENTS; ++j) {

                                    if ((z == 1) && (position[j]->x == x) && (position[j]->y == y) &&
                                    ...
                                    if ((z > 1) && (maze1[y][x].agentMovingTo[a][z - 1][j] == 1) &&
                                        (maze1[q][e].agentMovingTo[a][z][j] == 1)) {
                                        printf(" TRANSITION BETWEEN [%d %d] and [%d %d] at T %i, so To should be %i\n",
                                               y, x, q, e, z, maze1[q][e].toTransition[a][z]);
                                    }
    ...
    //Should compute here which of the neighboring cells are occupied by other agents
    ...
    distanceFromStart[a] = abs(position[a]->x - initialCellX[a]) + abs(position[a]->y - initialCellY[a]);
    ...
    //Compute my formula..
    int formula = (int) (hvalues[MAZEWIDTH * position[a]->y + position[a]->x][a]) + (2 * (lastMobileCellDist[a]) + 3);

    printf("My formula is %i and my pathlenght is %i ((H:) %i + 2* (lastmobilecell) %i +3)\n", formula, pathlength[a],
           (int) (hvalues[MAZEWIDTH * position[a]->y + position[a]->x][a]), lastMobileCellDist[a]);
...
//	printf("a:%d [%d,%d]\n",a, position[a]->y,position[a]->x);
    ...
    while (topheap2() != NULL) {
        ...
        //If in the middle of the search...
        if (cont_closed > 0) {
            printf("\nThe next node to expand is [%d %d] with depth %i (or %i) from parent [%d %d] (cont %i)",
                   tmpcell1->y, tmpcell1->x, tmpcell1->tmpdepth[a], newdepth, tmpcell3->y, tmpcell3->x, cont_closed);
            ...
            tmpcell1->searchtree = tmpcell1->tmpsearchtree[tmpcell1->depth[a]];//tmpcell3;


            // printf("The parent of [%d %d] is [%d %d] at depth %i..", tmpcell1->y, tmpcell1->x,tmpcell1->searchtree->y, tmpcell1->searchtree->x, tmpcell1->move[d]->depth[a]);
            //getchar();
        ...
        if ((tmpcell1 == mazegoal1) || (cont_closed == lookahead)) {   //open_size = opensize2() + open_size;
            ...
            for (d = 0; d < cont_closed; d++) {
                hvalues[MAZEWIDTH * CLOSED[d]->y + CLOSED[d]->x][a] = max(
                        hvalues[MAZEWIDTH * CLOSED[d]->y + CLOSED[d]->x][a], f_value - CLOSED[d]->g);

                //max(CLOSED[d]->h,f_value - CLOSED[d]->g);
                printf("Updating H of [%d %d] = %.1f, d: %i, size %i \n", CLOSED[d]->y, CLOSED[d]->x,
                       hvalues[MAZEWIDTH * CLOSED[d]->y + CLOSED[d]->x][a], d, sizeheap2());
        ...
        if (cont_closed > 1)  // Second to last step
        {
            printf("\n\n****** FROM CELL [%d %d] at time %i, H: %.1f, degree: %i\n ", tmpcell1->y, tmpcell1->x,
                   tmpcell1->depth[a], hvalues[MAZEWIDTH * tmpcell1->y + tmpcell1->x][a], tmpcell1->degree[a]);
        }


        if (cont_closed == 1) {// First state
            printf("\n\n******FROM CELL [%d %d] at time 0, H: %.1f, degree: %i\n ", tmpcell1->y, tmpcell1->x,
                   hvalues[MAZEWIDTH * tmpcell1->y + tmpcell1->x][a], tmpcell1->degree[a]);
        ...
        //        printf("A* generation a:%d [%d,%d] %d\n",a, tmpcell1->move[d]->y,tmpcell1->move[d]->x, d);
                if(tmpcell1->move[d]->g > tmpcell1->g + tmpcell1->cost[d])
                {
                    ...
                    tmpcell1->move[d]->key = (tmpcell1->move[d]->g + hvalues[MAZEWIDTH*tmpcell1->move[d]->y + tmpcell1->move[d]->x][a]) * BASE - tmpcell1->move[d]->g;
        ...
        for (i = 0; i < DIRECTIONS; ++i) {
            ...
            if (tmpcell1->move[d]) {
                ...
                if (!((tmpcell1->x == tmpcell1->move[d]->x) && (tmpcell1->y == tmpcell1->move[d]->y))) {
                    tmpcell1->degree[a] = tmpcell1->degree[a] + 1;
                }
                printf("\nThinking about moving to [%d %d]..(degree of parent %i) \n", tmpcell1->move[d]->y,
                       tmpcell1->move[d]->x, tmpcell1->degree[a]);
            ...
            if (tmpcell1->move[d] &&
                (!tmpcell1->move[d]->obstacle))//&& tmpcell1->move[d]->overexpanded != mazeiteration1)
            {


                initialize_state(tmpcell1->move[d]);
                printf("\nA* generation a:%d [%d,%d] from [%d %d], %.1f >= %.1f   ", a, tmpcell1->move[d]->y,
                       tmpcell1->move[d]->x, tmpcell1->y, tmpcell1->x, tmpcell1->move[d]->g,
                ...
                float magdir = sqrtf(((float) (tmpcell1->move[d])->x - (tmpcell1->x)) *
                                     ((float) (tmpcell1->move[d])->x - (tmpcell1->x)) +
                                     ((float) (tmpcell1->move[d])->y - (tmpcell1->y)) *
                                     ((float) (tmpcell1->move[d])->y - (tmpcell1->y)));


                if ((tmpcell1->move[d]->g >= tmpcell1->g +
                                             tmpcell1->cost[d]))//||((d==4)&&(tmpcell1->move[d]->g == tmpcell1->g + tmpcell1->cost[d]))) //way to check if state has been visited before
                {
                    ...
                    printf("\nThinking about moving to [%d %d] in my pathlength %i..\n", tmpcell1->move[d]->y,
                           tmpcell1->move[d]->x, pathlength[a]);
                    tmpcell1->move[d]->g_backup = (tmpcell1->g + tmpcell1->cost[d]);
                    tmpcell1->move[d]->g = (tmpcell1->g + tmpcell1->cost[d]);

                    determine_constraints(a, lookahead, formula, tmpcell1->move[d], 0);

                    //Check that, if another agent wants to move here in a later moment (greater or equal to pathlenght[a]) and it has the preference,
                    //I should be able to move out of the way, that is, to get to a position where I do not have evidence of another agent using this cell
                    ...
                    //After these three checks, we can see if agent can consider this move or not
                    ...
                    if (0)//(maxInfo!=a)//(maxHagent!=a)
                    {
                        ...
                        for (int j = 0; j < NAGENTS; ++j) {
                            if (canSee[a][j] > 0) {
                                /*   printf("CAN SEEE %i \n", j+1);
				   printf("The heuristic of my neighbor %i is %f " , j+1,hvalues[MAZEWIDTH*position[j]->y + position[j]->x][j]);
				  ...
				  */
                        ...
                        printf("Mi parent is [%d %d] at depth %i..", tmpcell1->y, tmpcell1->x,
                               tmpcell1->move[d]->tmpdepth[a]);
                        // getchar();
                        tmpcell1->move[d]->pathlength = tmpcell1->pathlength + 1;


                        //   pathlength[a]= abs(tmpcell1->move[d]->x-mazestart1->x)+ abs(tmpcell1->move[d]->y-mazestart1->y);

                        //     printf("PARENT OF [%d %d] is [%d %d] ...\n",tmpcell1->move[d]->y,tmpcell1->move[d]->x, tmpcell1->y, tmpcell1->x);

                        //      Computing     key
                        //tmpcell1->move[d]->key = (tmpcell1->move[d]->g + hvalues[MAZEWIDTH*tmpcell1->move[d]->y + tmpcell1->move[d]->x][a]) * BASE - tmpcell1->move[d]->g;

                        float tempG = (tmpcell1->g +
                                       tmpcell1->cost[d]);//*(tmpcell1->move[d]->penalty)/(float)tmpcell1->pathlength;



                        tmpcell1->move[d]->key =
                                (tempG + hvalues[MAZEWIDTH * tmpcell1->move[d]->y + tmpcell1->move[d]->x][a]) * BASE -
                                (tempG);
                        printf(" Adding [%d %d] with f %f to the heap ...\n", tmpcell1->move[d]->y,
                               tmpcell1->move[d]->x,
                               tempG + hvalues[MAZEWIDTH * tmpcell1->move[d]->y + tmpcell1->move[d]->x][a]);
                        insertheap2(tmpcell1->move[d]);
    ...
    if (flag_success1 == 1) {
        ...
        do {
            ...
            pathlength[a] = abs(cellpas->x - mazestart1->x) + abs(cellpas->y - mazestart1->y);
            ...
            if ((pathlength[a] > 0) && (co == 0)) {
                ...
                if (pasada == 1) {
                    ...
                    printf("IDEAL PATH [%d %d] at time %i, degree %i \n", idealPath[a][pathlength[a]]->y,
                           ...
                           maze1[idealPath[a][pathlength[a]]->y][idealPath[a][pathlength[a]]->x].degree[a]);
                }
            }
            //path[a][pathlength[a]]=cellpas;
            ...
            if (cellpas == mazestart1) {
                ...
                if (pathlength[a] == 1) {
                    ...
                    for (int b = 2; b <= lookahead; b++) {
                        path[a][b] = path[a][1];//NULL; //should enter here when agent wants to stay here
            ...
            while ((cellpas != mazestart1) || ((cellpas == mazestart1) && (pathlength[a] >= 1))) {
            ...
            printf("Checking cell [%d %d]...and [%d %d]", cellpas->y, cellpas->x, mazestart1->trace->y,
                   mazestart1->trace->x);
            ...
            if (mazestart1->trace != NULL) {
                ...
                if ((mazestart1->trace->blocked[pathlength[a]] != 1) ||
                    ((mazestart1->trace->y == cellpas->y) && (mazestart1->trace->x == cellpas->x) &&
                     (mazestart1->trace->blocked[pathlength[a]] ==
                      1)))         //&&(pathlength[a]>0))||((mazestart1->trace->blocked[pathlength[a]] == 1)&&(pathlength[a]==0)))
                {
                    printf(" GOT IT %i \n", flag_success1);
                    break;
                }
                ...
                if ((cellpas->blocked[pathlength[a]] != 1) || ((cellpas->blocked[pathlength[a]] == 1) && (cellpas ==
                                                                                                          mazestart1)))      //&&(pathlength[a]>0))||((mazestart1->trace->blocked[pathlength[a]] == 1)&&(pathlength[a]==0)))
                {
                    printf(" GOT IT %i \n", flag_success1);
                    printf("PATHLENGHT; %i \n", pathlength[a]);
                    break;
		     ...
		     * if (flag_success1 == 1)
    {
		...
		//	printf("construyendo path :%d agente %d\n",++co,a+1);
            ...
            printf(" PARECE QUE ESTOY BLOQUEADO?? NEXT [%d %d] with depth %i anda parent [%d %d]\n", cellpas->y,
                   cellpas->x, cellpas->tmpdepth[a], cellpas->tmpsearchtree[cellpas->tmpdepth[a]]->y,
...
int compute_constraintpath(int a, int lookahead) {
    ...
    for (int l = pathlength[a]; l >= 0; --l) {
        ...
        if ((pathlength[a] > l) && (conflictCost[a][idealPath[a][l]->y][idealPath[a][l]->x][l] <= 0.49) &&
            (conflictCost[a][idealPath[a][l]->y][idealPath[a][l]->x][l] > 0.01)
            && (limitcell == l) && (maze1[idealPath[a][l]->y][idealPath[a][l]->x].degree[a] < 3)) {
            //Unless it is the initial state, the agent does not have to move here as it will have to backtrack.
            //If it is the initial state: backtrack!
            if (l != 0) {
                limitcell = l - 1;
                printf("Cannot move here, will NOT violate deference constraints but the agent does not have enough mobility, limitcell is %i \n",
                       limitcell);
                //need to check the degree of this node, If it is less than 3 we are in trouble and need to go even further back
...
//getchar();
    ...
    printf("\nNOTHING HERE...first position in plan: [%d %d] vs [%d %d], second [%d %d] ", mazestart1->y, mazestart1->x,
           position[a]->y, position[a]->x, mazestart1->trace->y, mazestart1->trace->x);

    // New lookahead: lookahead -limitcell, represents how much should the agent compute
    // If new_lookahead = 0, means that the agent is free to move in its own idealPath.
    ...
    printf("\n1 NOTHING HERE...first position in plan: [%d %d] vs [%d %d], second [%d %d] ", mazestart1->y,
           mazestart1->x, position[a]->y, position[a]->x, mazestart1->trace->y, mazestart1->trace->x);

    printf("\n2 NOTHING HERE...first position in plan: [%d %d] vs [%d %d], second [%d %d] ", mazestart1->y,
           mazestart1->x, position[a]->y, position[a]->x, mazestart1->trace->y, mazestart1->trace->x);
...
//	printf("a:%d [%d,%d]\n",a, position[a]->y,position[a]->x);
        ...
        printf("\n3 NOTHING HERE...first position in plan: [%d %d] vs [%d %d] ", mazestart1->y, mazestart1->x,
               position[a]->y, position[a]->x);


        mazeiteration1++;

        //printf("\n4 NOTHING HERE...first position in plan: [%d %d] vs [%d %d], second [%d %d] ",mazestart1->y, mazestart1->x, position[a]->y, position[a]->x,mazestart1->trace->y,mazestart1->trace->x );


        emptyheap2();

        //printf("\n5 NOTHING HERE...first position in plan: [%d %d] vs [%d %d], second [%d %d] ",mazestart1->y, mazestart1->x, position[a]->y, position[a]->x,mazestart1->trace->y,mazestart1->trace->x );
    ...
    while ((topheap2() != NULL) && (new_lookahead != 0)) {
        ...
        //This next block of code (if statement) should not be executed in this path planning, as we do not want to overwrite previously found heuristic values
        ...
        if ((tmpcell1 == mazegoal1) || (cont_closed == lookahead)) {   //open_size = opensize2() + open_size;
            ...
            for (d = 0; d < cont_closed; d++) {
                ...
                //hvalues[MAZEWIDTH*CLOSED[d]->y + CLOSED[d]->x][a] = max(hvalues[MAZEWIDTH*CLOSED[d]->y + CLOSED[d]->x][a],f_value - CLOSED[d]->g);
        ...
        for (i = 0; i < DIRECTIONS; ++i) {
            ...
            if (tmpcell1->move[d]) {
                ...
                printf("\nThinking about moving to [%d %d]..whose ConflictCost is %.1f\n", tmpcell1->move[d]->y,
                       tmpcell1->move[d]->x,
                       conflictCost[a][tmpcell1->move[d]->y][tmpcell1->move[d]->x][pathlength[a]]);

                printf("****\nPROSPECTIVE NODE [%d %d] is at time/depth %i..is it MARKED? %i\n ******",
                       tmpcell1->move[d]->y, tmpcell1->move[d]->x, newdepth,
                       maze1[tmpcell1->move[d]->y][tmpcell1->move[d]->x].marked[a][newdepth]);
                //tmpcell1->move[d]->depth[a], maze1[tmpcell1->move[d]->y][tmpcell1->move[d]->x].marked[a][tmpcell1->move[d]->depth[a]]) ;
            ...
            if (tmpcell1->move[d] && (!tmpcell1->move[d]->obstacle) &&
                (!maze1[tmpcell1->move[d]->y][tmpcell1->move[d]->x].marked[a][newdepth]))   //tmpcell1->move[d]->depth[a]&& tmpcell1->move[d]->overexpanded != mazeiteration1)
            {
                                             ...
                                             tmpcell1->cost[d]))//||((d==4)&&(tmpcell1->move[d]->g == tmpcell1->g + tmpcell1->cost[d]))) //way to check if state has been visited before
                {
                    ...
                    for (int future = pathlength[a]; future < lookahead; ++future) {
                        ...
                        if (maze1[tmpcell1->move[d]->y][tmpcell1->move[d]->x].blockedAgent[a][future]) {
                            ...
                            printf("QUICK SCORE: %f",
                                   conflictCost[a][tmpcell1->move[d]->y][tmpcell1->move[d]->x][pathlength[a]]);//(float)1/(float)(future-pathlength[a]+2));
                    ...
                    int maxHagent = a;
                    int maxInfo = a;
                    if ((maze1[tmpcell1->move[d]->y][tmpcell1->move[d]->x].blockedAgent[a][pathlength[a]]) &&
                        (pathlength[a] > 0)) //Another agent wants to move here
                    {
                        ...
                        int numConflicts = 0;
                        float maxH = backupH[MAZEWIDTH * (position[a]->y) +
                                             (position[a]->x)][a];//backupH[MAZEWIDTH*(tmpcell1->move[d]->y) + (tmpcell1->move[d]->x)][a];//hvalues[MAZEWIDTH*(position[a]->y) + (position[a]->x)][a];//[MAZEWIDTH*(tmpcell1->y) + (tmpcell1->x)][a];
                        float sumH = 0;

                        for (int j = 0; j < NAGENTS; ++j) {

                            if (maze1[tmpcell1->move[d]->y][tmpcell1->move[d]->x].agentMovingTo[a][pathlength[a]][j] >
                                0) {
                                printf("This guy -> %i  (total %i) inGoal?: %i or (%i-%i)=%i \n", j + 1, numConflicts,
                                       goal_reached[j], hvalues[MAZEWIDTH * position[j]->y + position[j]->x][j],
                                       pathlength[a],
                                       (hvalues[MAZEWIDTH * position[j]->y + position[j]->x][j] - pathlength[a]));

                                if ((goal_reached[j]) ||
                                    ((hvalues[MAZEWIDTH * position[j]->y + position[j]->x][j] - pathlength[a]) < 0.1)) {
                                ...
                                printf("His previous position at [%d %d] had a degree of %i \n",
                                       path[j][pathlength[a]]->y, path[j][pathlength[a]]->x,
                                       maze1[path[j][pathlength[a]]->y][path[j][pathlength[a]]->x].degree[j]);
                                ...
                                for (int l = 1; l <= lookahead; l++) {
                                    if ((a != j) && (path[j][l] != NULL)) {
                                        printf("Agent %i: [%d %d], H: %.1f, step %i \n", j + 1, path[j][l]->y,
                                               path[j][l]->x, hvalues[MAZEWIDTH * position[j]->y + position[j]->x][j],
                                               l);

                                        if ((position[a]->y == path[j][l]->y) && (position[a]->x == path[j][l]->x)) {
                                            //conflictsPosition++;
                                            position[a]->numConflicts[a] = numConflicts;//position[a]->numConflicts[a]+1;
                                ...
                                printf(" with H of %.1f, (%.1f)  vs ",
                                       hvalues[MAZEWIDTH * (tmpcell1->move[d]->y) + (tmpcell1->move[d]->x)][j],
                                       hvalues[MAZEWIDTH * (position[j]->y) + (position[j]->x)][j]);

                                printf(" my H of %.1f, (%.1f) ",
                                       hvalues[MAZEWIDTH * (tmpcell1->move[d]->y) + (tmpcell1->move[d]->x)][a],
                                       hvalues[MAZEWIDTH * (position[a]->y) +
                                               (position[a]->x)][a]);//[MAZEWIDTH*(tmpcell1->y) + (tmpcell1->x)][a]);
                    ...
                    // printf(" Bloqueado? %i pathlength %i goal reached %i (agent %i) ", maze1[tmpcell1->move[d]->y][tmpcell1->move[d]->x].blockedAgent[a][0],pathlength[a],goal_reached[i],i+1);
                    int canmovehere = 1;
                    for (int u = 0; u < NAGENTS; ++u) {
                        if ((tmpcell1->move[d]->x == position[u]->x) && (tmpcell1->move[d]->y == position[u]->y) &&
                            (pathlength[a] == 1) && (u != a)) {
                            printf("\nOps, agent %i is at the next position position, is it its goal?..", u + 1);
                    ...
                    if ((((maze1[tmpcell1->move[d]->y][tmpcell1->move[d]->x].blockedAgent[a][0])) &&
                         (pathlength[a] == 1)) && (!canmovehere)) //Another agent IS here (first step)
                    {
                        printf("\n****At %i Cant move to [%d %d], there is an agent\n", pathlength[a],
                               tmpcell1->move[d]->y, tmpcell1->move[d]->x);
                        ...
                        for (int j = 0; j < NAGENTS; ++j) {
                            ...
                            if ((tmpcell1->move[d]->y == position[j]->y) && (tmpcell1->move[d]->x == position[j]->x)) {

                                (tmpcell1->move[d])->numConflicts[a] = (tmpcell1->move[d])->numConflicts[a] + 1;
                                printf("This guy -> %i, total conflicts %i \n", j + 1,
                        ...
                        (pathlength[a] > 1)) // Another agent might ALREADY be here (> first step)
                    {
                        ...
                        for (int j = 0; j < NAGENTS; ++j) {
                            ...
                            if (maze1[tmpcell1->move[d]->y][tmpcell1->move[d]->x].agentMovingTo[a][pathlength[a] -
                                                                                                   1][j] > 0) {
                                numConflicts++;
                                printf("This guy -> %i  (total %i)", j + 1, numConflicts);
				  ...
				  */
                        ...
                        float tempG = (tmpcell1->g + tmpcell1->cost[d]) + 3 *
                                                                          (conflictCost[a][tmpcell1->move[d]->y][tmpcell1->move[d]->x][pathlength[a]]);//*(tmpcell1->move[d]->penalty)/(float)tmpcell1->pathlength;
        ...
        do {
            ...
            while ((cellpas != mazestart1) || ((cellpas == mazestart1) && (pathlength[a] >= 1))) {
                ...
                if ((pathlength[a] > 0) && (co == 0)) {
                    ...
                    printf("Att [%d %d] at time %i \n", path[a][pathlength[a]]->y, path[a][pathlength[a]]->x,
                           pathlength[a]);
                    if (pasada == 1) {
                        //idealPath[a][pathlength[a]]=path[a][pathlength[a]];
                    }
...
void
observe_new_agents(int a, int i, int lookahead) {   //printf(" seen at [%d %d] ", track[a][i][0][1],track[a][i][0][0]);
    ...
    if (track[a][i][0][0] == -1) // if a hasn't seen agent i before
    {
        ...
        if ((!goal_reached[i]) &&
            ...
            printf("Agent %i appeared AGAIN?? [%d %d] can see agent %i at [%d %d]\n", a + 1, position[a]->y,
                   position[a]->x, i + 1, position[i]->y, position[i]->x);
        ...
        if (((abs(position[i]->x - position[a]->x) + abs(position[i]->y - position[a]->y)) > (lookahead)) &&
            ...
            for (int z = 0; z < (lookahead); z++) {
                ...
                if (maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].blockedAgent[a][z] >
                    0)// (blockedAgent[mostProbPositionXY[a][i][z][0]][mostProbPositionXY[a][i][z][1]][a][z]>0)
                {
                    maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].blockedAgent[a][z] =
                            maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].blockedAgent[a][z] -
                            1;
                    ...
                    if (z > 0) {
                        ...
                        maze1[mostProbPositionXY[a][i][z - 1][1]][mostProbPositionXY[a][i][z -
                                                                                           1][0]].fromTransition[a][z]--;
//betweenTransition[mostProbPositionXY[a][i][z-1][0]][mostProbPositionXY[a][i][z-1][1]][mostProbPositionXY[a][i][z][0]][mostProbPositionXY[a][i][z][1]][a][z]--;
                        ...
                        maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].agentMovingTo[a][z][i] = 0;
                        //agentMovingTo[mostProbPositionXY[a][i][z][0]][mostProbPositionXY[a][i][z][1]][a][z][i]=0;
                        maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].toTransition[a][z]--;

                        printf("(%i) ANNNd transition between [%d %d] and  [%d %d] back to  %i\n", z,
                               mostProbPositionXY[a][i][z - 1][1], mostProbPositionXY[a][i][z - 1][0],
...
void observe_agent2(int a, int i, int lookahead, cell1 *previous) {
    ...
    if ((!goal_reached[i]) &&
        (((abs(position[i]->x - position[a]->x) + abs(position[i]->y - position[a]->y)) <= (lookahead)) &&
         ((abs(position[i]->x - position[a]->x) + abs(position[i]->y - position[a]->y)) > 0))) {
        canSee[a][i] = 1;
        printf("Agent %i at [%d %d] can see agent %i at [%d %d]\n", a + 1, position[a]->y, position[a]->x, i + 1,
               position[i]->y, position[i]->x);
        totp++;
        if ((mostProbPositionXY[a][i][1][0] == position[i]->x) && (mostProbPositionXY[a][i][1][1] == position[i]->y)) {
            ...
            //printf("increasing one to bad predictions, now is %i with lookahead %i", badpredictions[lookahead], lookahead);
        }
        ...
        for (int z = 0; z < (lookahead); z++) {
            //printf(" HERE I AM %i %d %d",!goal_reached[i],mostProbPositionXY[a][i][z][1],mostProbPositionXY[a][i][z][0]);//,maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].blockedAgent[a][z] );

            if ((mostProbPositionXY[a][i][z][0] > -1) && (mostProbPositionXY[a][i][z][1] > -1)) {
                if (maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].blockedAgent[a][z] > 0) {
                    maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].blockedAgent[a][z] =
                            maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].blockedAgent[a][z] -
                            1;
                    ...
                    if (z > 0) {
                        //    betweenTransition[mostProbPositionXY[a][i][z-1][0]][mostProbPositionXY[a][i][z-1][1]][mostProbPositionXY[a][i][z][0]][mostProbPositionXY[a][i][z][1]][a][z]--;
                        maze1[mostProbPositionXY[a][i][z - 1][1]][mostProbPositionXY[a][i][z - 1][0]].fromTransition[a][
                                z - 1]--;
...
void updateHistory(int a, int i, int lookahead, cell1 *previous) {
    ...
    if (track[a][i][1][0] != -1) //The agent has at least two observations of its neighbor i
    {
        if ((track[a][i][1][1] == track[a][i][0][1]) && (track[a][i][1][0] == track[a][i][0][0])) {


        } else {
            //blockedAgent[track[a][i][1][0]][track[a][i][1][1]][a][0]=	blockedAgent[track[a][i][1][0]][track[a][i][1][1]][a][0]-1;
        }
        ...
        for (int z = 0; z < (lookahead); z++) {
            ...
            maze1[position[i]->y][position[i]->x].blockedAgent[a][z] = 1;// blockedAgent[position[i]->x][position[i]->y][a][z]=1;
            ...
            if (z > 0) {
                ...
                printf("Observing AGNET %i moving FROM [%d %d] at t %i TO [%d %d] at t %i, total: %i\n", i,
                       position[i]->y, position[i]->x, z - 1, position[i]->y, position[i]->x,
                       z, maze1[position[i]->y][position[i]->x].toTransition[a][z]);
...
void evalPrevPrediction(int a, int i, int correct) {
    ...
    printf(" and with neighbor %i, it is %.2f, or %i of total %i\n", i + 1,
           (float) good_pred_agents[a][i] / (float) pred_agents[a][i], good_pred_agents[a][i], pred_agents[a][i]);
...
void updateProbabilities(int a, int i) {
    ...
    int total_nextCell = obsNextCell[a][i][2] + obsNextCell[a][i][0] + obsNextCell[a][i][1] + obsNextCell[a][i][3] +
                         obsNextCell[a][i][4];
    ...
    printf("Observed Probabilities: DOWN %.1f, UP %.1f, LEFT %.1f , RIGHT %.1f\n", nextCellProb[a][i][1],
           nextCellProb[a][i][3], nextCellProb[a][i][2], nextCellProb[a][i][0]);
...
void computePrediction2(int a, int i, int lookahead) {
    printf("AGENT %i EEERASING PREVIOUS OBSERVATIONS of %i as in [%d %d] REALDEPTH %i, lookahead %i\n", a + 1, i + 1,
           mostProbPositionXY[a][i][0][1], mostProbPositionXY[a][i][0][0], realDepth[i], lookahead);
    ...
    { //printf("Now z is %i and %i\n", z,mostProbPositionXY[a][i][z][0]);
        if (mostProbPositionXY[a][i][z][0] > -2) {
            if (z == 0) {
                ...
                maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].blockedAgent[a][z] = 0;
                //blockedAgent[mostProbPositionXY[a][i][z][0]][mostProbPositionXY[a][i][z][1]][a][z]=0;
                printf("\nERASING blocking at [%d %d] to %i  ", mostProbPositionXY[a][i][z][1],
                       mostProbPositionXY[a][i][z][0],
                       maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].blockedAgent[a][z]);

//printf("how about this [%d %d] %i at 0 and this [%d %d] %i at 1 ",mostProbPositionXY[a][i][0][1],mostProbPositionXY[a][i][0][0], maze1[mostProbPositionXY[a][i][0][1]][mostProbPositionXY[a][i][0][0]].blockedAgent[a][0],
                //mostProbPositionXY[a][i][1][1],mostProbPositionXY[a][i][1][0]  ,maze1[mostProbPositionXY[a][i][1][1]][mostProbPositionXY[a][i][1][0]].blockedAgent[a][1]);
            ...
            if ((z > 0) &&
                (maze1[mostProbPositionXY[a][i][z - 1][1]][mostProbPositionXY[a][i][z - 1][0]].fromTransition[a][z -
                                                                                                                 1] >
                 0) &&
                (maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].toTransition[a][z] > 0)) {
                ...
                maze1[mostProbPositionXY[a][i][z - 1][1]][mostProbPositionXY[a][i][z - 1][0]].fromTransition[a][z -
                                                                                                                1]--;
                //betweenTransition[mostProbPositionXY[a][i][z-1][0]][mostProbPositionXY[a][i][z-1][1]][mostProbPositionXY[a][i][z][0]][mostProbPositionXY[a][i][z][1]][a][z]--;
                printf("\nERASING TRANSITION BETWEEN [%d %d] and [%d %d] to %i  ", mostProbPositionXY[a][i][z - 1][1],
                ...
                printf("\nERASING TRANSITION TO [%d %d] to %i (at %i)", mostProbPositionXY[a][i][z][1],
                       mostProbPositionXY[a][i][z][0],
                       maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].toTransition[a][z], z);


            }
            if (maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].toTransition[a][z] ==
            ...
            {
                maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].blockedAgent[a][z] =
                        maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].blockedAgent[a][z] - 1;
                ...
                printf("\n1 No more ToTrans, ERASING BLOCKED [%d %d] at t %i to %i  ", mostProbPositionXY[a][i][z][1],
                       mostProbPositionXY[a][i][z][0], z,
                       maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].blockedAgent[a][z]);
            }
    ...
    //printf("\nACCORDING TO AGENT %i, MOST LIKELY DIRECTION FOR AGENT %i, currently at [%d %d], at time 1 IS: ", a+1, i+1, position[i]->y, position[i]->x);
...
//	blockedAgent[mostProbPositionXY[a][i][0][0]][mostProbPositionXY[a][i][0][1]][a][0]=1;
    ...
    for (int t = 1; t < realDepth[i]; t++)//(lookahead);t++)
    {
        ...
        maze1[mostProbPositionXY[a][i][t][1]][mostProbPositionXY[a][i][t][0]].blockedAgent[a][t] =
                maze1[mostProbPositionXY[a][i][t][1]][mostProbPositionXY[a][i][t][0]].blockedAgent[a][t] + 1;
...
//	blockedAgent[mostProbPositionXY[a][i][t][0]][mostProbPositionXY[a][i][t][1]][a][t]=blockedAgent[mostProbPositionXY[a][i][t][0]][mostProbPositionXY[a][i][t][1]][a][t]+1;
        ...
        if (maze1[mostProbPositionXY[a][i][t - 1][1]][mostProbPositionXY[a][i][t - 1][0]].fromTransition[a][t - 1] <
            0) {
            maze1[mostProbPositionXY[a][i][t - 1][1]][mostProbPositionXY[a][i][t - 1][0]].fromTransition[a][t - 1] = 0;
        }
...
//	 toTransition[mostProbPositionXY[a][i][t][0]][mostProbPositionXY[a][i][t][1]][a][t]);
        ...
        printf("TRANSITION: [%i %i] to [%i %i] at time %i: %i \n", mostProbPositionXY[a][i][t - 1][1],
               mostProbPositionXY[a][i][t - 1][0], mostProbPositionXY[a][i][t][1], mostProbPositionXY[a][i][t][0],
               t,
               maze1[mostProbPositionXY[a][i][t - 1][1]][mostProbPositionXY[a][i][t - 1][0]].fromTransition[a][t - 1]);
        //betweenTransition[mostProbPositionXY[a][i][t-1][0]][mostProbPositionXY[a][i][t-1][1]][mostProbPositionXY[a][i][t][0]][mostProbPositionXY[a][i][t][1]][a][t]);

        //printf("REDUCING POS: [%i %i] at time %i to %i \n",mostProbPositionXY[a][i][t-1][1], mostProbPositionXY[a][i][t-1][0],t-1,blockedAgent[mostProbPositionXY[a][i][t-1][0]][mostProbPositionXY[a][i][t][1]][a][t-1]);
...
void computePrediction(int a, int i, int lookahead) {
        ...
        if (mostProbPositionXY[a][i][z][0] > -2) {
            if (z == 0) {
                ...
                printf("\nERASING blocking at [%d %d] to %i  ", mostProbPositionXY[a][i][z][1],
                       mostProbPositionXY[a][i][z][0],
                       maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].blockedAgent[a][z]);
                printf("how about this [%d %d] %i at 0 and this [%d %d] %i at 1 ", mostProbPositionXY[a][i][0][1],
                       mostProbPositionXY[a][i][0][0],
                       maze1[mostProbPositionXY[a][i][0][1]][mostProbPositionXY[a][i][0][0]].blockedAgent[a][0],
                ...
                0) //    (toTransition[mostProbPositionXY[a][i][z][0]][mostProbPositionXY[a][i][z][1]][a][z]==0)//(blockedAgent[mostProbPositionX[a][i][z]][mostProbPositionY[a][i][z]][a][z]>0)
            {
                ...
                printf("\n2 No more ToTrans, ERASING BLOCKED [%d %d] at t %i to %i  ", mostProbPositionXY[a][i][z][1],
                       mostProbPositionXY[a][i][z][0], z,
                       maze1[mostProbPositionXY[a][i][z][1]][mostProbPositionXY[a][i][z][0]].blockedAgent[a][z]);
            }
    ...
    mostProbPositionXY[a][i][0][0] = (position[i]->x);
    mostProbPositionXY[a][i][0][1] = (position[i]->y);
    printf("\nACCORDING TO AGENT %i, MOST LIKELY DIRECTION FOR AGENT %i, currently at [%d %d], at time 0 IS: ", a + 1,
           i + 1, position[i]->y, position[i]->x);
    printf("SAME!!: [%i %i]\n", mostProbPositionXY[a][i][0][1], mostProbPositionXY[a][i][0][0]);
...
//	blockedAgent[mostProbPositionXY[a][i][0][0]][mostProbPositionXY[a][i][0][1]][a][0]=1;
    ...
    //printf("how about at 4?: %i", blockedAgent[mostProbPositionX[a][i][4]][mostProbPositionY[a][i][4]][a][4]);

    if (pred_agents[a][i] > 0) {

        //If prediction accuracy is too low (for now <0.5) don't make prediction, assume it will not bother the agent
    ...
    //printf("SO: rate, %.1f from %i and %i and predict is %i \n", (float)good_pred_agents[a][i]/(float)pred_agents[a][i],good_pred_agents[a][i], pred_agents[a][i],predict[a][i]);
    //	getchar();
    ...
    for (int t = 1; t < (lookahead); t++) {
        ...
        //printf("ACCORDING TO AGENT %i, MOST LIKELY DIRECTION FOR AGENT %i, currently at [%d %d], at time %i IS: ", a+1, i+1, position[i]->y, position[i]->x,t);
        //printf("Most prob is %i ", mostProb);
        ...
        if (mostProb == 0) {
            ...
            if (t == 1) {
                ...
                if ((mostProbPositionXY[a][i][t - 1][0] + 1) < MAZEWIDTH) {
                    ...
                    if (maze1[mostProbPositionXY[a][i][t - 1][1]][mostProbPositionXY[a][i][t - 1][0] + 1].obstacle ==
                        0) {
            ...
            if ((((position[i]->x) + 1) >= MAZEWIDTH) || (mostProbPositionXY[a][i][t - 1][0] + 1 >= MAZEWIDTH)) {
                mostProbPositionXY[a][i][t][0] = mostProbPositionXY[a][i][t - 1][0];
                mostProbPositionXY[a][i][t][1] = mostProbPositionXY[a][i][t - 1][1];
                printf("STUCKK!!: [%i %i]\n", mostProbPositionXY[a][i][t][1], mostProbPositionXY[a][i][t][0]);
            } else {
        ...
        if (mostProb == 1) {
            ...
            if (t == 1) {
                ...
                if ((mostProbPositionXY[a][i][t - 1][1] + 1) < MAZEHEIGHT) {
                    ...
                    if (maze1[mostProbPositionXY[a][i][t - 1][1] + 1][mostProbPositionXY[a][i][t - 1][0]].obstacle ==
                        0) {
            ...
            if ((((position[i]->y + 1)) >= MAZEHEIGHT) || (mostProbPositionXY[a][i][t - 1][1] + 1 >= MAZEHEIGHT)) {
	...
	* */
        ...
        if (mostProb == 3) {
            ...
            if ((((position[i]->y - 1)) < 0) || (mostProbPositionXY[a][i][t - 1][1] - 1 < 0)) {
...
//	 toTransition[mostProbPositionXY[a][i][t][0]][mostProbPositionXY[a][i][t][1]][a][t]);
        ...
        printf("TRANSITION: [%i %i] to [%i %i] at time %i: %i \n", mostProbPositionXY[a][i][t - 1][1],
               mostProbPositionXY[a][i][t - 1][0], mostProbPositionXY[a][i][t][1], mostProbPositionXY[a][i][t][0],
               t, maze1[mostProbPositionXY[a][i][t - 1][1]][mostProbPositionXY[a][i][t - 1][0]].fromTransition[a][t]);
        //betweenTransition[mostProbPositionXY[a][i][t-1][0]][mostProbPositionXY[a][i][t-1][1]][mostProbPositionXY[a][i][t][0]][mostProbPositionXY[a][i][t][1]][a][t]);
...
void randommove(int a) {
    ...
    int i;
    for (i = 0; i < DIRECTIONS; ++i) {
        if (tmpcell1->move[d] && (!tmpcell1->move[d]->obstacle) && (!tmpcell1->move[d]->blocked[0])) {
...
//			i = random() % NAGENTS;
        ...
        for (i = 0; i < NAGENTS; i++) {
            ...
            if (RUN1 >= 0 && robot_steps1 >= 0) {
                ...
                Multi_print_grid();
                for (k = 0; k < NAGENTS; k++) {
                    printf("(%d)[%d,%d]....(%i and %i) ", k + 1, position[k]->y, position[k]->x, role[0][1],
                           role[1][0]);
            ...
            if (position[i] != goal[i]) {  //While it is not at its goal...
                ...
                if (!compute_shortestpath_astar(i, lookahead)) {
                    ...
                    for (int l = 0; l <= lookahead; l++) {
                        ...
                        if (idealPath[i][l] != NULL) {
                            ...
                            if ((l > 0) && (idealPath[i][l - 1] != NULL)) {
                                //If the conflict cost is too high, make it 1 to emphasize the problem of visiting this cell
                            ...
                            printf("IDEAL PATH AT POS %i: [%d %d] - CCost %.2f\n", l, idealPath[i][l]->y,
                                   idealPath[i][l]->x, conflictCost[i][idealPath[i][l]->y][idealPath[i][l]->x][l]);
                            pathlength[i] = l;
                        }
                    }
                    ...
                    if (!compute_constraintpath(i, lookahead)) {
                        ...
                        if ((position[i]->parent[i]->blocked[0]) && (position[i]->parent[i]->x != position[i]->x) &&
                        ...
                        if ((position[i]->trace->blocked[0]) && (position[i]->trace->x != position[i]->x) &&
						...
						{
							previous->velx[i]=(float)(position[i]->x - previous->x)*ALPHA + (float)previous->velx[i]*(1-ALPHA);
							previous->vely[i]=(float)(position[i]->y - previous->y)*ALPHA + (float)previous->vely[i]*(1-ALPHA);

						}


						printf("**DV of cell [%d %d]: [%.1f %.1f] for ag. %i\n",previous->y,previous->x,previous->vely[i],previous->velx[i], i+1);
						 */
                        //	getchar();
                        ...
                        if (position[i] == goal[i]) {
                            ...
                            printf("** LLEGO time_step:%d** %d finish:%d cost:%f total cost:%f, now at [%d %d]\n",
                                   time_step, i, NAGENTS - finish_all, agent_cost[i], total_cost, position[i]->y,
                                   position[i]->x);
                            //getchar();
#endif
                            if (finish_all == 0) {
...
void call_rtaastar() {
    ...
    float total_score[(int) (sizeof(look) / (float) sizeof(int))], avg_score[(int) (sizeof(look) /
                                                                                    (float) sizeof(int))], total_time[(int) (
            sizeof(look) / (float) sizeof(int))];
    float avg_finish[(int) (sizeof(look) / (float) sizeof(int))], last_finish[(int) (sizeof(look) /
                                                                                     (float) sizeof(int))];
    srand(time(NULL));
    float ftimes[RUNS];
//for (prunning = 0; prunning <1; prunning++)
    for (i = 0; i < (int) (sizeof(look) / (float) sizeof(int)); i++) {
        ...
        for (RUN1 = 0; RUN1 < RUNS; ++RUN1) {
            ...
            for (int a = 0; a < NAGENTS; a++) {
                ...
                for (int j = 0; j < NAGENTS; j++) {
                    lastMobileCellDist[a] = 10000;//big number to emphasize that at the beginning he does not have memory of previously mobile states
                    pred_agents[a][j] = 0;
                    agentInfo[a] = -1;
            ...
            avg_finish[i] = (avg_finish[i] * RUN1 + (float) (NAGENTS - finish_all)) / (float) (RUN1 + 1);

            if ((NAGENTS - finish_all) > 0) {
                last_finish[i] = (last_finish[i] * RUN1_agents + (float) lastfinish) / (float) (RUN1_agents + 1);
            ...
            printf(" Bad predictions: %i, Good: %i and total: %i, rate: %.1f \n", badp, goop, totp,
                   (float) goop / (float) totp);
            ftimes[RUN1] = lastfinish;
            for (int a = 0; a < NAGENTS; a++) {
                for (int j = 0; j < NAGENTS; j++) {
                    printf("\nGoodPred for %i with %i: %.2f (%i/%i)", a + 1, j + 1,
                           (float) good_pred_agents[a][j] / (float) pred_agents[a][j], good_pred_agents[a][j],
                           pred_agents[a][j]);
            ...
            fprintf(salida, "%d %f %d %d %d %f %d %d %d %d", lookahead, solution_cost, NAGENTS, NAGENTS - finish_all,                    searches_astar1, (time_astar - time_astar_initialize1) * 1000, time_step - 1, RUN1, statexpanded1,
                    statpercolated2);
            ...
            for (int a = 0; a < NAGENTS; a++) {
                for (int j = 0; j < NAGENTS; j++) {
                    ...
                    if (j != 0) {
                        ...
                        //printf("Acc to %i, AGENT %i moved UP: %i, DOWN: %i, LEFT: %i, RIGHT:%i \n", a+1, j+1, obsNextCell[a][j][1], obsNextCell[a][j][3], obsNextCell[a][j][2],  obsNextCell[a][j][0]);
                    }
                }
            }
    ...
    for (int i = 0; i < (int) (sizeof(look) / (float) sizeof(int)); i++) {
        ...
        for (int a = 0; a < NAGENTS; a++) {
            for (int j = 0; j < NAGENTS; j++) {
                if (j != 0) {
                    //	printf("Acc to %i, AGENT %i moved UP: %i, DOWN: %i, LEFT: %i, RIGHT:%i \n", a, j, obsNextCell[a][j][1], obsNextCell[a][j][3], obsNextCell[a][j][2],  obsNextCell[a][j][0]);
                }
            }
        }
